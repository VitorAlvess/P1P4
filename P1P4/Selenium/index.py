...
class scrappy:
    ...
    def configuracoes_atados(self):
        ...
        options.set_preference("browser.download.dir", f'{os.getcwd()}\\relatorios')
        ...
        self.navegar = webdriver.Firefox(service=FirefoxService(GeckoDriverManager().install()), options=options)
    ...
    def pagina_opipa(self):
        ...
        sleep(2)
        try:
            pagina.find_element('xpath','/html/body/reach-portal/div[3]/div/div/div/div/div/button[2]' ).click()
        except:
            pass
        sleep(5)
        self.vagas = pagina.find_elements('xpath','/html/body/div/div[2]/div[1]/div/div/div[3]/div/div/div[2]/table/tbody/tr') #Clica em gerenciar vagas
        ...
        while i < len(self.vagas): 
            ...
            sleep(5)
            vaga_nome = pagina.find_elements('xpath',f'/html/body/div/div[2]/div[1]/div/div/div[3]/div/div/div[2]/table/tbody/tr/td[1]/div/a[1]/span')
            ...
            pagina.execute_script(f"window.scrollTo(0, {i + 1}80)")
            acima = f'{i + 1}80'
            if i > 10:
                pagina.execute_script(f"window.scrollTo(0, {int(acima) + 200})")
            sleep(10)
            try:
                pagina.find_element('xpath','/html/body/reach-portal/div[3]/div/div/div/div/div/button[1]' ).click()
                pagina.find_element('xpath','/html/body/reach-portal/div[3]/div/div/div/div/div/button[2]' ).click()
            ...
            pagina.find_element('xpath',f'/html/body/div/div[2]/div[1]/div/div/div[3]/div/div/div[2]/table/tbody/tr[{1+i}]/td[5]/a' ).click()
            ...
            try:
                ...
                pagina.find_element('xpath','//*[@id="voluntarios"]/div[1]/div[4]/button' ).click()
    ...
    def sheets(self):
        ...
        try:
            ...
            for i in range(len(self.vagas)):  # Total de planilhas / Range vai ser definido pelo numero total de vagas ativas
                if i == 0:
                    workbook = openpyxl.load_workbook('relatorios/voluntarios.xlsx')  # O primeiro arquivo
                else:
                    workbook = openpyxl.load_workbook(f'relatorios/voluntarios({i}).xlsx')  # Arquivos subsequentes

                worksheet = workbook['Usuários Inscritos']  # Nome da aba

                for row in worksheet.iter_rows(min_row=2, values_only=True):  # Comece da segunda linha (linhas == 1) para evitar o cabeçalho
                    valores_adicionar = []
                    for colunas in range(1, 10):  # Iterar pelas colunas de 1 a 8
                        if colunas == 4:
                            valor = row[colunas - 1].split()[0]
                            valores_adicionar.append(valor)  # Insira os valores na lista   

                        elif colunas != 6 and colunas !=7:
                            valor = row[colunas - 1]
                            valores_adicionar.append(valor)  # Insira os valores na lista   
                    valores_total.append(valores_adicionar)
            print(valores_total)
            print('Valores total acima')
            # Certifique-se de fechar os arquivos após a leitura, se necessário
            workbook.close()
